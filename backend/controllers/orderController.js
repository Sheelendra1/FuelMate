        ...
        try {
            ...
            for (const admin of admins) {
                await createNotification(
                    admin._id,
                    'New Order Received',
                    `New order #${order.orderId} received from ${customer.name}. Amount: ₹${finalAmount || totalAmount}`,
                    'transaction',
            ...
            await createNotification(
                ...
                `Your order #${order.orderId} has been placed successfully. Please proceed to payment.`,
                'transaction',
                { orderId: order.orderId }
            );
        } catch (notifError) {
    ...
    try {
        ...
        if (customer) {
            ...
            await createNotification(
                customer._id,
                'Order Completed',
                `Your order #${order.orderId} has been completed! You earned ${order.pointsEarned} points.`,
                'transaction',
                { orderId: order.orderId }
            );
        }
    ...
    try {
        ...
        try {
            await createNotification(
                order.customerId,
                'Order Cancelled',
                `Your order #${order.orderId} has been cancelled. Reason: ${reason || 'Admin cancelled'}.`,
                'transaction',
                { orderId: order.orderId }
            );
        } catch (err) {
